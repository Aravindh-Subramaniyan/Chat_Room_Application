...
def client_exit(client, client_ip, client_port):
    ...
    open("/home/shanthini/Desktop/user_pass.txt", 'w').close()              #Clear user list & password in saved file
    stdout.flush()     
    client.close()
...
def prompt_create_username(client_sock):
    client_sock.sendall('Welcome to Messenger! Are you a new user? [yes/no]')
    ...
    if (response == "yes"):
        ...
        with open('/home/shanthini/Desktop/user_pass.txt', 'a') as aFile:
...
def populate_logins_dictionaries():
    ...
    with open('/home/shanthini/Desktop/user_pass.txt') as aFile:
        for line in aFile:
            (key, val) = line.split()
            user_logins[key] = val
...
def main(argv):
    ...
    try:
        ...
        stdout.flush()
        open("/home/shanthini/Desktop/user_pass.txt", 'w').close()                  #On keyboard interrupt EXIT
        print '\nServer shut down.'
